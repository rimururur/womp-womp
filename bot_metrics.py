"""
The bot_metrics module offers functionality for monitoring Autocatcher statistics.
Also generating reports from data.
"""
...
class BotMetrics:
    """
    The monitoring statistics class
    Metrics:
    ...
    def checkpoint(self):
        """
        Generates a metrics checkpoint that also tracks the elapsed time.
        """
        elapsed = (datetime.now() - self.start_time).total_seconds()
        self.logger.pprint(
            f"{get_formatted_time(elapsed)} have passed.\n"
            "Formulating a reference point to optimize metrics",
            color="blue",
            timestamp=True
        )
    ...
    async def checkpointer(self, duration: int = 300):
        """
        Implements an async handler to checkpoint the stats monitor.
        """
